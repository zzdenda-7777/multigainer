...
public class MessageManager {
    ...
    public String getJoinMessage(Player player, boolean isNewPlayer) {
        ...
        if (isNewPlayer) {
            ...
            return "§8[§e§lNEW §6PLAYER§l§8]§f Welcome to the server " + playerName + "! #welcome" + joinNumber;
        } else {
            return "§8[§a§l+§8]§f " + playerName + " joined the game!";
        }
    }

    public String getLeaveMessage(Player player) {
        String playerName = player.getName();
        return "§8[§4§l-§8]§f" + playerName + " left the game!";
    }
